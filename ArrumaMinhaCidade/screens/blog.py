...
class BlogScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # Carrega status do JSON, se disponível
    ...
    def on_pre_enter(self, *args):
        """Recarrega os status do JSON antes de entrar na tela."""
        self.load_status_from_json()
        self.update_posts(self.service_updates)

    def load_status_from_json(self):
        """Carrega os status dos serviços do arquivo JSON."""
        try:
        ...
        updated = False
        for service in self.service_updates:
            service_title = service['title']
            service_data = all_updates.get(service_title, {})
            
            # Verifica se service_data é uma lista (formato antigo)
            if isinstance(service_data, list):
                print(f"Convertendo formato antigo para {service_title}.")
                all_updates[service_title] = {
                    'status': service['status'],  # Usa o status padrão do service_updates
        ...
        # Salva o JSON atualizado se houve conversão
        if updated:
