...
class BlogScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.seguindo_servicos = set()  # Conjunto para armazenar IDs dos serviços seguidos
        ...
        # Label do usuário (será atualizado em on_pre_enter)
        ...
        for update in updates:
            ...
            def toggle_follow(btn, update=update):
                ...
                if update['id'] in self.seguindo_servicos:
                    self.seguindo_servicos.remove(update['id'])
                    btn.text = 'Seguir'
                else:
                    self.seguindo_servicos.add(update['id'])
                    btn.text = 'Deixar de Seguir'

                # Atualiza o campo "seguindo" do usuário logado e salva no JSON
                if hasattr(app, "usuario_logado") and app.usuario_logado:
                    app.usuario_logado["seguindo"] = list(self.seguindo_servicos)
                ...
                # Sincroniza imediatamente o conjunto com o usuário logado
                self.seguindo_servicos = set(app.usuario_logado.get("seguindo", []))
            follow_btn.bind(on_press=toggle_follow)
            text_layout.add_widget(follow_btn)

            post.add_widget(text_layout)
            self.posts_layout.add_widget(post)

    def on_post_touch_down(self, instance, touch):
        # Se o clique foi em um botão filho, não faz nada (deixa o botão funcionar)
        for child in instance.children:
            if isinstance(child, Button) and child.collide_point(*touch.pos):
                return False
            # Se o child for um layout, verifica os filhos dele também
    ...
    def on_leave(self):
        """Salva os serviços seguidos no App quando a tela é deixada."""
        app = App.get_running_app()
        if hasattr(app, "usuario_logado"):
            app.usuario_logado["seguindo"] = list(self.seguindo_servicos)

            # Após adicionar/remover o serviço em app.usuario_logado["seguindo"]
            usuarios = carregar_todos_usuarios()
            for user in usuarios:
                if user["cpf"] == app.usuario_logado["cpf"]:
                    user["seguindo"] = app.usuario_logado["seguindo"]
                    break
            salvar_todos_usuarios(usuarios)
