    ...
    def submit_admin_update(self, instance):
        ...
        if update_text:
            ...
            service_data['updates'].append({
                'id': self.update_data.get('id', '000AAA'),  # Adiciona o ID do serviço
                'text': update_text,
            ...
            # Envia notificação para usuários seguindo o serviço
            app = App.get_running_app()
            service_id = self.current_service_id 
            usuarios = carregar_todos_usuarios()
            for user in usuarios:
                if "seguindo" in user and service_id in user["seguindo"]:
                    if "notificacoes" not in user:
                        user["notificacoes"] = []
                    user["notificacoes"].append({
            ...
            salvar_todos_usuarios(usuarios)  # Função que salva todos usuários no JSON
